...
int main(int argc, char** argv)
{
	if(argc < 2)
	{
		printf("Please enter number of corners for Polygon!\n");
		exit(-1);
	}

	int corners = atoi(argv[1]);
	
	if(corners < 3)
	{
		printf("The input must be a number and at least 3!\n");
		exit(-1);
	}
	

	printf("Hello polygon!\n");
	...
	GLFWwindow* window = glfwCreateWindow(user_data.window_width, user_data.window_height, "Polygon", NULL, NULL);
	...
	init_gl(window, corners);

	while (!glfwWindowShouldClose(window))
	{
		// Draw the next frame:
		draw_gl(window, corners);
